...
const INDIA_DATA = [
  ...
  { state: "Arunachal Pradesh", districts: ["Anjaw","Changlang","Dibang Valley","East Kameng","East Siang","Kra Daadi","Kurung Kumey","Lohit","Longding","Lower Dibang Valley","Lower Siang","Lower Subansiri","Namsai","Papum Pare","Siang","Tawang","Tirap","Upper Dibang Valley","Upper Siang","Upper Subansiri","West Kameng","West Siang"] },
  { state: "Assam", districts: ["Baksa","Barpeta","Biswanath","Bongaigaon","Cachar","Charaideo","Chirang","Darrang","Dhemaji","Dhubri","Dibrugarh","Dima Hasao","Goalpara","Golaghat","Hailakandi","Hojai","Jorhat","Kamrup Metropolitan","Kamrup","Karbi Anglong","Karimganj","Kokrajhar","Lakhimpur","Majuli","Morigaon","Nagaon","Nalbari","Sivasagar","Sonitpur","South Salmara-Mankachar","Tinsukia","Udalguri","West Karbi Anglong"] },
  ...
  { state: "Punjab", districts: ["Amritsar","Barnala","Bathinda","Faridkot","Fatehgarh Sahib","Fazilka","Ferozepur","Gurdaspur","Hoshiarpur","Jalandhar","Kapurthala","Ludhiana","Malerkotla","Mansa","Moga","Mohali","Muktsar","Pathankot","Patiala","Rupnagar","Sangrur","Shahid Bhagat Singh Nagar","Tarn Taran"] },
  ...
  try {
    ...
    for (const item of INDIA_DATA) {
      ...
      if (existing.rows.length > 0) {
        // Update districts if state already exists
        await pool.query(
          'UPDATE states_districts SET districts = $1, updated_at = NOW() WHERE LOWER(state_name) = LOWER($2)',
          [item.districts, item.state]
        )
        skipped++
      } else {
        await pool.query(
          'INSERT INTO states_districts (state_name, districts) VALUES ($1, $2)',
          [item.state, item.districts]
        )
        inserted++
      }
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Done! Inserted ${inserted}, updated ${skipped} states.`,
      total: INDIA_DATA.length
    })
  } catch (error) {

...
export default function BdmConversationPage() {
  const [contacts, setContacts] = useState<Contact[]>([])
  const [currentUserName, setCurrentUserName] = useState<string>('Prerna')
  ...
  const fetchContacts = useCallback(async (selectDefault = false) => {
    try {
      const res = await fetch('/api/admin/conversation?portal=bdm')
      const data = await res.json()
      if (data.success) {
        const cUser = data.currentUser?.name || 'Prerna'
  ...
  const fetchMessages = useCallback(async (contactName: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/conversation/${encodeURIComponent(contactName)}?portal=bdm`)
      const data = await res.json()
      if (data.success) {
  ...
  const markAsRead = useCallback(async (contactName: string) => {
    if (!contactName) return
    try {
      await fetch(`/api/admin/conversation/${encodeURIComponent(contactName)}?portal=bdm`, { method: 'PUT' })
      const res = await fetch('/api/admin/conversation?portal=bdm')
      const data = await res.json()
      if (data.success) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (activeContact) {
        fetch(`/api/admin/conversation/${encodeURIComponent(activeContact)}?portal=bdm`)
          .then(r => r.json())
          .then(data => {
        ...
        fetch('/api/admin/conversation?portal=bdm')
          .then(r => r.json())
          .then(data => {
            if (data.success) {
              const cUser = data.currentUser?.name || currentUserName
                  ...
                  {loadingMessages && messages.length === 0 ? (
                    ...
                    messages.map((msg, index) => {
                      const isOutgoing = msg.is_outgoing !== undefined 
                        ? msg.is_outgoing 
                        : (msg.sender === currentUserName || msg.sender === 'Prerna')
                      return (
                        <div
                          key={msg.id || index}
                          ...
                          <div
                            className={`px-4 py-3 rounded-2xl text-sm leading-relaxed shadow-sm font-medium ${
                              isOutgoing
                                ...
                                : 'bg-slate-200/80 text-slate-800 dark:bg-slate-700 dark:text-slate-100 rounded-tl-none border border-slate-100 dark:border-slate-700'
                            }`}
                          >
                            {msg.message}
                          </div>

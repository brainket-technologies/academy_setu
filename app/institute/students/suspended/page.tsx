    ...
    <div className="flex flex-col gap-6 max-w-[1600px] mx-auto w-full pb-10 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-4">
          <h1 className="text-2xl font-black text-slate-800 dark:text-slate-100">Suspended Students</h1>
          <div className="flex items-center gap-2 border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-1.5 bg-slate-50 dark:bg-slate-700/50">
            <span className="text-sm font-bold text-slate-600 dark:text-slate-300">Total Suspended</span>
            <span className="text-sm font-black text-sky-500">03</span>
          </div>
        </div>
            ...
            <table className="w-full text-sm text-center border-collapse whitespace-nowrap min-w-[950px]">
              <thead>
                <tr className="bg-sky-50/50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                  ...
                  <th className="py-4 px-4 font-bold text-slate-600 dark:text-slate-300">Suspended At</th>
                  <th className="py-4 px-4 font-bold text-slate-600 dark:text-slate-300">Action</th>
                </tr>
              </thead>
              <tbody>
                {suspendedStudents.map((student, index) => (
                  <tr key={student.id} className="border-b border-slate-100 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                    ...
                    <td className="py-4 px-4 font-semibold text-slate-600 dark:text-slate-300">{student.rollNo}</td>
                    ...
                    <td className="py-4 px-4 font-semibold text-slate-600 dark:text-slate-300">{student.fee}</td>
                    <td className="py-4 px-4 font-semibold text-slate-600 dark:text-slate-300">{student.remark}</td>
                    <td className="py-4 px-4">
                      <div className="flex flex-col items-center justify-center gap-1">
                        ...
                        <div className="text-slate-500 text-[11px] font-semibold">@ {student.createTime}</div>
                      </div>
                    </td>
                    <td className="py-4 px-4">
      ...
      {showFilterModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white rounded-3xl p-8 max-w-5xl w-full shadow-2xl relative animate-in zoom-in-95 duration-200">
            <button onClick={() => setShowFilterModal(false)} className="absolute right-6 top-6 w-8 h-8 flex items-center justify-center rounded-full border border-slate-200 text-slate-400 hover:bg-slate-50 transition-colors">
              <X className="w-4 h-4" />
            </button>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-8 mt-4 mb-10">
              {[
                ...
                { label: 'Stream', placeholder: 'Select Stream' },
                { label: 'Gender', placeholder: 'Select Gender' },
                { label: 'Category', placeholder: 'Select Category' },
                { label: 'Sort By', placeholder: 'Select an Option' },
                { label: 'RTE Student', placeholder: 'Select an Option' },
                { label: 'Child With Special Needs', placeholder: 'Select an Option' },
                { label: 'Referred By', placeholder: 'Select Category' },
                { label: 'House/Block', placeholder: 'Select an Option' },
                { label: 'Is Migrated?', placeholder: 'Select an Option' },
                { label: 'Having Balance?', placeholder: 'Select an Option' },
                { label: 'Discount Head', placeholder: 'Select an Option' },
                { label: 'Tag', placeholder: 'Select Tag' },
              ].map((field) => (
                <div key={field.label}>
            ...
            <div className="flex items-center justify-center gap-4">
              <button onClick={() => setShowFilterModal(false)} className="px-10 py-2.5 bg-teal-600 text-white font-bold rounded-xl hover:bg-teal-700 transition-colors shadow-sm">Filter</button>
              <button onClick={() => setShowFilterModal(false)} className="px-10 py-2.5 border border-teal-600 text-teal-600 font-bold rounded-xl hover:bg-teal-50 transition-colors">Clear</button>
            </div>
          </div>
        </div>
      )}

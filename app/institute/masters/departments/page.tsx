...
export default function DepartmentsPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('school_masters_departments')
    if (saved) {
      try {
      ...
      localStorage.setItem('school_masters_departments', JSON.stringify(INITIAL_DEPARTMENTS))
    }
  }, [])
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    const newDep: DepartmentRecord = {
      id: Date.now(),
      departmentName: deptNameInput.trim(),
    ...
    localStorage.setItem('school_masters_departments', JSON.stringify(updated))
  ...
  const handleOpenEdit = (item: DepartmentRecord) => {
    setSelectedDept(item)
    setDeptNameInput(item.departmentName)
    setParentNameInput(item.parentName === '—' ? '' : item.parentName)
    setEditModalOpen(true)
  }

  const handleEditSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!selectedDept || !deptNameInput) return
  ...
  const handleDelete = (id: number) => {
    const updated = departments.map(d => d.id === id ? { ...d, deleted: true } : d)
    setDepartments(updated)
    localStorage.setItem('school_masters_departments', JSON.stringify(updated))
    showToast('Department moved to deleted list!')
  }

  const handleRestore = (id: number) => {
    const updated = departments.map(d => d.id === id ? { ...d, deleted: false } : d)
    setDepartments(updated)
    localStorage.setItem('school_masters_departments', JSON.stringify(updated))
    showToast('Department restored successfully!')
  }

  const tabCountAll = departments.filter(d => !d.deleted).length
  const tabCountDeleted = departments.filter(d => d.deleted).length

  // Filter logic
  const filtered = departments.filter(d => {
    const matchesTab = activeTab === 'All' ? !d.deleted : d.deleted
    const matchesSearch = searchQuery ? d.departmentName.toLowerCase().includes(searchQuery.toLowerCase()) : true
    const matchesParent = parentFilter ? d.parentName === parentFilter : true
    return matchesTab && matchesSearch && matchesParent
  })
  ...
  return (
    <div className="flex flex-col gap-6 w-full pb-10 animate-in fade-in duration-300">
      ...
      <div className="bg-white border rounded-2xl p-4 shadow-sm flex items-center justify-between">
        <h1 className="text-xl font-black text-slate-800">Departments</h1>
        <button
          onClick={() => {
            setDeptNameInput('')
            setParentNameInput('')
      ...
      {showFilters && (
        <div className="bg-white border rounded-2xl p-5 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-semibold text-slate-700 animate-in slide-in-from-top-3 duration-200">
          <div className="flex flex-col gap-1.5">
            <label className="text-slate-500 font-bold">Search Department</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-slate-500 font-bold">Filter by Parent Group</label>
            <select
              value={parentFilter}
          ...
          <table className="w-full text-center border-collapse">
            <thead className="bg-slate-50 font-black text-slate-655 border-b">
              <tr>
                ...
                <th className="px-3 py-4 text-left">Department Name</th>
                <th className="px-3 py-4">Parent Name</th>
            ...
            <tbody>
              {filtered.map((item, idx) => (
                <tr key={item.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50 transition-colors font-semibold">
                  ...
                  <td className="px-3 py-3.5 text-left font-bold text-slate-800">{item.departmentName}</td>
                  <td className="px-3 py-3.5 text-slate-600">{item.parentName}</td>
      ...
      {addModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white border rounded-3xl shadow-2xl w-full max-w-md p-6 text-xs font-semibold text-slate-700 animate-in zoom-in-95 duration-200 relative">
            ...
            <h2 className="text-sm font-black text-[#1b3a60] border-b pb-2 mb-4">Add Department</h2>
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-slate-500 font-bold">Department Name</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-slate-500 font-bold">Parent Group</label>
                <select
                  value={parentNameInput}
                  onChange={e => setParentNameInput(e.target.value)}
                  className="w-full px-4 py-2 border rounded-lg outline-none bg-white font-bold"
                >
      ...
      {editModalOpen && selectedDept && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white border rounded-3xl shadow-2xl w-full max-w-md p-6 text-xs font-semibold text-slate-700 animate-in zoom-in-95 duration-200 relative">
            ...
            <h2 className="text-sm font-black text-[#1b3a60] border-b pb-2 mb-4">Edit Department</h2>

...
const ADMISSION_CHART = [
  { id: 1, class: 'Class VII', studentType: 'New', amount: '1000/-', date: '16/06/2025', time: '11:00 AM' },
  { id: 2, class: 'Class VIII', studentType: 'Old', amount: '2000/-', date: '16/06/2025', time: '11:00 AM' },
  { id: 3, class: 'Class IX', studentType: 'All', amount: '1500/-', date: '16/06/2025', time: '11:00 AM' },
]

export default function AdmissionFeePage() {
  const [activeTab, setActiveTab] = useState('Applicable Student')
  ...
  return (
    <div className="flex flex-col gap-6 max-w-[1200px] mx-auto w-full pb-10 animate-in fade-in duration-300">
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 shadow-sm flex items-center justify-between">
        <h1 className="text-xl font-black text-slate-800 dark:text-slate-100">Admission Fee</h1>
      ...
      {showForm && (
        ...
        <h2 className="text-sm font-black text-slate-700 uppercase tracking-wider mb-6 pb-2 border-b border-slate-200">Create Admission Fee Classwise</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-slate-700">Student Type <span className="text-red-500">*</span></label>
            <div className="flex items-center gap-4 py-2.5">
              <label className="flex items-center gap-1.5 text-sm text-slate-600 font-semibold cursor-pointer">
                <input type="radio" name="studentType" defaultChecked className="accent-teal-600" /> All
              </label>
              <label className="flex items-center gap-1.5 text-sm text-slate-600 font-semibold cursor-pointer">
                <input type="radio" name="studentType" className="accent-teal-600" /> New
              </label>
              <label className="flex items-center gap-1.5 text-sm text-slate-600 font-semibold cursor-pointer">
                <input type="radio" name="studentType" className="accent-teal-600" /> Old
              </label>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-center">
          <button className="px-8 py-2.5 rounded-lg bg-teal-600 text-white text-sm font-bold hover:bg-teal-700 transition-colors shadow-sm">
            Create
          </button>
        </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <h2 className="text-lg font-black text-slate-800 dark:text-slate-100">All Admission Fee</h2>
        ...
        <div className="flex flex-wrap items-center gap-4 mb-6">
          <button onClick={() => setActiveTab('Admission Fee Chart')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[11px] font-bold border transition-colors ${
            activeTab === 'Admission Fee Chart' ? 'bg-purple-100 text-purple-700 border-purple-200' : 'bg-purple-50 text-purple-600 border-purple-100 hover:bg-purple-100'
          }`}>
            Admission Fee Chart <span className="bg-white text-purple-700 px-1.5 py-0.5 rounded text-[10px]">03</span>
          </button>
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700 relative min-h-[400px]">
          ...
          {activeTab === 'Admission Fee Chart' && (
            <table className="w-full text-xs text-center">
              <thead className="bg-slate-50 dark:bg-slate-800/80 font-black text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700">
                <tr>
                  <th className="px-3 py-3">S. No.</th>
                  <th className="px-3 py-3">Class</th>
                  <th className="px-3 py-3">Student Type</th>
              ...
              <tbody>
                {ADMISSION_CHART.map((item, i) => (
                  <tr key={item.id} className="border-b border-slate-100 dark:border-slate-700/50 last:border-0 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-3 py-4 text-slate-700 font-semibold">{item.class}</td>
                    <td className="px-3 py-4 text-slate-600">{item.studentType}</td>
                    <td className="px-3 py-4 text-slate-700 font-bold">{item.amount}</td>
          ...
          {activeTab !== 'Applicable Student' && activeTab !== 'Admission Fee Chart' && (

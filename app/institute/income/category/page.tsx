...
export default function IncomeCategoryPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      localStorage.setItem('income_categories', JSON.stringify(INITIAL_CATEGORIES))
    }
  }, [])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
      alert('Please fill in Income Name.')
      return
    }
  ...
  return (
    <div className="flex flex-col gap-6 w-full pb-10 animate-in fade-in duration-300">
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 shadow-sm">
        <h1 className="text-xl font-black text-slate-800 dark:text-slate-100">Income Category</h1>
        <p className="text-xs text-slate-400">Classify operational incomes for ledger bookkeeping reports</p>
      </div>

      {/* Add Category Form (Screenshot 3) */}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs font-semibold text-slate-700">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-1.5">
              <label className="font-bold text-slate-700">Income Name *</label>
              <input 
                type="text" 
          ...
          <table className="w-full text-xs text-center border-collapse">
            <thead className="bg-slate-50 dark:bg-slate-800/80 font-black text-slate-655 border-b">
              <tr>
                ...
                <th className="px-4 py-4 text-left">Income Name</th>

...
import {
  ...
  Key, MapPin, User, Lock, Shield, Plus, FileText, Image, ExternalLink
} from 'lucide-react'
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
              ...
              <div className="md:col-span-4 bg-white dark:bg-slate-800 border border-slate-200/50 dark:border-slate-700/60 rounded-2xl shadow-sm overflow-hidden flex flex-col self-stretch">
                ...
                <div className="p-6 flex flex-col gap-4 text-sm flex-1 justify-center">
                  ...
                  <div>
                    <span className="block text-[10px] uppercase font-bold text-slate-400 tracking-wider mb-0.5">Role Login URL</span>
                    {(() => {
                      const normalized = (formData.role || '').trim().toLowerCase()
                      const loginUrl = normalized === 'admin' ? '/admin/login' : normalized === 'manager' ? '/manager/login' : normalized === 'bdm' ? '/bdm/login' : `/${normalized}/login`
                      return (
                        <a
                          href={loginUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center justify-between w-full px-3.5 py-2 border border-indigo-200/70 dark:border-indigo-800/70 bg-indigo-50/50 dark:bg-indigo-950/40 rounded-xl font-mono text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100/60 transition-colors group cursor-pointer"
                        >
                          <span>{loginUrl}</span>

...
export default function CreateInstitutePage() {
  ...
  // Computed state options strictly from Settings (/api/admin/settings/state-city)
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const response = await fetch('/api/admin/institute', {
        method: 'POST',
      ...
      if (resData.success) {
        ...
        toast.error(resData.error || 'Failed to create institute.')
      }
    } catch (err) {
      ...
      <div className="flex flex-col gap-6 max-w-5xl mx-auto w-full pb-10">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl px-8 py-5 border border-slate-100 dark:border-slate-700 shadow-sm flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Create Institute</h1>
          </div>
        </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="relative w-full">
                    <input
                      ...
                      placeholder="Enter a password for the institute to log in (Optional)"
                      value={password}

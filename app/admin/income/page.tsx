...
export default function AllIncomePage() {
  ...
  const fetchAuxiliaryData = async () => {
    try {
      const [catRes, partRes, sessRes] = await Promise.all([
        fetch('/api/admin/income/categories'),
        fetch('/api/admin/income/parties'),
        fetch('/api/admin/income/sessions')
      ])
  ...
  const fetchRecords = useCallback(async (page = 1, search = '', activeF = activeFilters) => {
    ...
    try {
      ...
      if (activeF.session) params.append('session', activeF.session)
      if (activeF.received_from) params.append('received_from', activeF.received_from)

      const res = await fetch(`/api/admin/income/records?${params.toString()}`)
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!addForm.income_category || !addForm.amount || !addForm.payment_mode || !addForm.received_from) {
      toast.error('Please fill in all required fields')
      return
    }
    setSubmittingAdd(true)
    try {
      const res = await fetch('/api/admin/income/records', {
        method: 'POST',
      ...
      if (data.success) {
        ...
        toast.error(data.error || 'Failed to add income record')
      }
    } catch {
      toast.error('Error adding income record')
    } finally {
  ...
  const handleOpenEdit = (rec: IncomeRecord) => {
    ...
    const formattedDate = new Date(rec.received_date).toISOString().split('T')[0]
    setEditForm({
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (!editForm.income_category || !editForm.amount || !editForm.payment_mode || !editForm.received_from) {
      toast.error('Please fill in all required fields')
      return
    }
    setSubmittingEdit(true)
    try {
      const res = await fetch(`/api/admin/income/records/${editingRecordId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleOpenView = (rec: IncomeRecord) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/income/records/${deleteTargetId}`, {
        method: 'DELETE'
      })
      const data = await res.json()
      if (data.success) {
  ...
  const handleExport = () => {
    try {
      const headers = ['S.No.', 'Reference No.', 'Income Type', 'Amount', 'Payment Mode', 'Income Date', 'Received From', 'Paid To', 'Status']
      const csvRows = [headers.join(',')]
      records.forEach((row, i) => {
        const dateStr = new Date(row.received_date).toLocaleDateString()
        const values = [
          String(i + 1),
          `"${row.trans_id}"`,
          `"${row.income_category}"`,
      ...
      a.setAttribute('download', `income_records_${new Date().toISOString().split('T')[0]}.csv`)
      a.click()
    ...
    <AdminLayout>
      <div className="flex flex-col gap-6 p-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">All Income</h1>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Manage, filter, and track all incoming school revenue streams.</p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-700 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 bg-slate-50/50 dark:bg-slate-800/50">
                  ...
                  <th className="py-4 px-6">Income Date</th>
                  <th className="py-4 px-6">Received From</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700 text-sm text-slate-600 dark:text-slate-300">
                {loading ? (
                  ...
                  records.map((row, idx) => {
                    ...
                    const dateStr = new Date(row.received_date).toLocaleDateString('en-GB')
                    return (
                      <tr key={row.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="py-4 px-6 font-medium text-slate-700 dark:text-slate-300">{row.income_category}</td>
                        <td className="py-4 px-6 font-bold text-slate-800 dark:text-slate-100">₹{Number(row.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                        ...
                        <td className="py-4 px-6 font-semibold text-slate-800 dark:text-slate-200">{row.received_from}</td>
                        <td className="py-4 px-6 font-medium text-slate-500 dark:text-slate-400">{row.payment_account || '-'}</td>
      ...
      {isFilterModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 dark:bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-150">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-2xl p-6 w-full max-w-lg relative flex flex-col animate-in zoom-in-95 duration-200 z-10">
            {/* Circle Close button matching Screenshot 3 */}
            <button
              onClick={() => setIsFilterModalOpen(false)}
              className="absolute top-4 right-4 p-1 rounded-full border border-slate-200 dark:border-slate-700 text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-slate-600 transition-colors cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-350 mb-1.5">Income Category</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-350 mb-1.5">Session</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-350 mb-1.5">Income Parties</label>
                  <select
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 dark:bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-150">
          ...
          >
            ...
            <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-6">Add Income</h3>

            <div className="flex flex-col gap-6">
              {/* Section 1: Income Details */}
              <div>
                <h4 className="text-sm font-bold text-slate-750 dark:text-slate-300 border-b border-slate-100 dark:border-slate-700 pb-2 mb-4">Income Details</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Income Category *</label>
                    <select
                      required
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">Income Date *</label>
                    <input
                      type="date"
                      required
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 dark:bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-150">
          ...
          >
            ...
            <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-6">Edit Income</h3>
      ...
      {/* VIEW INCOME RECORD DETAILS MODAL */}
            ...
            <div className="flex flex-col gap-5">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <span className="font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-1">
                      <Layers className="w-3.5 h-3.5 text-slate-400" /> {viewingRecord.income_category}
                    </span>
                  </div>
                  <div>
                    <span className="block text-[10px] font-bold text-slate-400 uppercase mb-0.5">Income Date</span>
                    <span className="font-medium text-slate-800 dark:text-slate-200 flex items-center gap-1">
                      <Calendar className="w-3.5 h-3.5 text-slate-400" /> {new Date(viewingRecord.received_date).toLocaleDateString('en-GB')}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-y-4 gap-x-4 text-sm">
                  ...
                  <div>
                    <span className="block text-[10px] font-bold text-slate-400 uppercase mb-0.5">Payment Account (Paid To)</span>
                    <span className="font-medium text-slate-800 dark:text-slate-200">
                      {viewingRecord.payment_account || 'N/A'}
                    </span>
                  </div>
                  <div>
                    <span className="block text-[10px] font-bold text-slate-400 uppercase mb-0.5">Received From</span>
                    <span className="font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-1">
                      <User className="w-3.5 h-3.5 text-slate-400" /> {viewingRecord.received_from}
                    </span>
                  </div>
                  <div>
                    <span className="block text-[10px] font-bold text-slate-400 uppercase mb-0.5">Session</span>
                    <span className="font-medium text-slate-800 dark:text-slate-200">
                      {viewingRecord.session_name || 'N/A'}
                    </span>
                  </div>
                  <div>
              ...
              {/* Attachment */}
              <div>
                <span className="block text-[10px] font-bold text-slate-400 uppercase mb-2">Photo / Receipt</span>

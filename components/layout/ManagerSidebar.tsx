...
import { managerLogoutAction, getManagerSessionAction } from '@/app/manager/login/actions'
        ...
        <div className="p-5 shrink-0 flex items-center justify-between border-b border-white/20 dark:border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] font-semibold text-slate-400 dark:text-slate-500 tracking-widest mt-0.5 uppercase">Manager Portal</p>
            </div>
          </div>
          {onClose && (
            <button 
            ...
            if (hasSubItems) {
              return (
                <div key={i}>
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 cursor-pointer ${
                      parentActive
                        ...
                        : 'text-slate-600 dark:text-slate-300 hover:bg-white/40 dark:hover:bg-white/5 hover:text-blue-600 dark:hover:text-blue-400'
                    }`}
                    style={parentActive ? {
                  ...
                  {isMenuOpen && (
                     <div className="mt-1 ml-3 pl-4 border-l-2 border-blue-200/60 dark:border-blue-800/40 flex flex-col gap-0.5 py-1">
                          ...
                          <Link
                            ...
                            className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-[12px] font-semibold transition-all duration-150 ${
                              exactActive
                                ...
                                : 'text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-white/40 dark:hover:bg-white/5'
                            }`}
                          >
                            <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${exactActive ? 'bg-blue-500' : 'bg-slate-300 dark:bg-slate-600'}`} />
                            {sub.label}
                          </Link>
                        )
                      })}

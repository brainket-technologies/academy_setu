...
import { getManagerSessionAction, managerLogoutAction } from '@/app/manager/login/actions'
import Link from 'next/link'

export function ManagerHeader({ onMenuToggle }: { onMenuToggle?: () => void }) {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        const session = await getManagerSessionAction()
    ...
    const updatePhoto = () => {
      const stored = localStorage.getItem('managerProfilePhoto')
      if (stored) {
    ...
    return () => {
      clearInterval(timer)
      window.removeEventListener('profileUpdated', updatePhoto)
    }
  }, [])
  ...
  return (
    <header
      ...
      style={{
        ...
        boxShadow: '0 1px 24px rgba(59,130,246,0.06)',
      }}
    >
      ...
      <div className="flex items-center gap-2.5">
        ...
        {mounted && (
          <button
            ...
            className="p-2 text-slate-500 dark:text-slate-400 hover:text-blue-500 dark:hover:text-blue-400 hover:bg-white/50 dark:hover:bg-white/10 border border-white/40 dark:border-white/10 rounded-xl shadow-sm transition-all cursor-pointer hover:scale-105 active:scale-95"
            title="Toggle theme"
          >
        ...
        <div className="relative" ref={profileRef}>
          <div
            ...
            className="flex items-center gap-2 pl-2.5 pr-2 py-1.5 rounded-2xl transition-all duration-200 cursor-pointer group border select-none"
            style={{
          ...
          >
            <div className="text-right hidden sm:block">
              <p className="text-[12px] font-bold text-slate-800 dark:text-slate-100 leading-none group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{userName}</p>
              <p className="text-[10px] text-slate-400 dark:text-slate-500 font-semibold mt-0.5">Manager</p>
            </div>
            <div className="relative">
              <div className="w-8 h-8 rounded-xl overflow-hidden border-2 border-white/80 dark:border-white/20 shadow-sm transition-transform group-hover:scale-105"
                style={{ background: 'linear-gradient(135deg, #3b82f6, #0284c7)' }}>
          ...
          {isProfileOpen && (
            <div className="absolute right-0 top-full mt-2 w-52 bg-white dark:bg-slate-800 rounded-2xl border border-slate-150 dark:border-slate-700 shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150 z-50">
              <div className="p-3 border-b border-slate-100 dark:border-slate-700/80 bg-slate-50/50 dark:bg-slate-800/50">
                ...
                <p className="text-[10px] text-slate-400 dark:text-slate-400 font-medium truncate">{userEmail || 'Manager Account'}</p>
              </div>
              ...
              <div className="p-1.5 flex flex-col gap-0.5">
                <Link 
                  ...
                  className="flex items-center gap-2.5 px-3 py-2 text-[13px] font-semibold text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50/70 dark:hover:bg-blue-950/30 rounded-xl transition-colors cursor-pointer"
                >
